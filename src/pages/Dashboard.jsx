...
export default function Dashboard() {
  ...
  const stats = [
    { label: 'Total Users',   value: users.length,    icon: FiUsers,     bg: 'bg-blue-50',   color: 'text-blue-600',   link: '/dashboard/users' },
    { label: 'Total Results', value: results.length,  icon: FiFileText,  bg: 'bg-green-50',  color: 'text-green-600',  link: '/dashboard/results' },
    { label: 'Admins',        value: adminCount,       icon: FiTrendingUp, bg: 'bg-purple-50', color: 'text-purple-600', link: '/dashboard/users' },
    { label: 'Exam Types',    value: 3,                icon: FiActivity,  bg: 'bg-orange-50', color: 'text-orange-600', link: '/dashboard/performance' },
  ];
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="px-4 sm:px-6 py-4 border-b border-gray-100 flex items-center justify-between">
          ...
          <Link to="/dashboard/results" className="text-sm text-blue-600 hover:underline font-medium whitespace-nowrap">
            View all →
          </Link>
        </div>
              ...
              {recent.map((r, i) => (
                <div key={i} className="px-4 py-3 flex items-center justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <span className={`mt-1 inline-block px-2 py-0.5 rounded-full text-xs font-semibold ${typeBadge[r.type] || 'bg-gray-100 text-gray-500'}`}>
                      {typeLabels[r.type] || r.type}
                    </span>
                  </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
        <Link
          to="/dashboard/users"
          className="bg-blue-600 hover:bg-blue-700 text-white rounded-2xl p-4 sm:p-5 flex items-center gap-4 transition-all shadow-sm"
        >
        ...
        <Link
          ...
          className="bg-white hover:shadow-md text-gray-700 border border-gray-100 rounded-2xl p-4 sm:p-5 flex items-center gap-4 transition-all shadow-sm"
        >
