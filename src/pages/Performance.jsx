...
export default function Performance() {
  ...
  const numericEntries = levelData ? Object.entries(levelData).filter(([k]) => k !== 'Total') : [];

  const startEdit  = (key, val) => { setEditingKey(key); setEditBuf({ ...val }); setAddingNew(false); };
  const cancelEdit = () => { setEditingKey(null); setEditBuf({}); };
  const commitEdit = (key) => { setLevelData((p) => ({ ...p, [key]: { ...editBuf } })); setEditingKey(null); setEditBuf({}); };
  const deleteEntry = (key) => setLevelData((p) => { const c = { ...p }; delete c[key]; return c; });
  const commitAdd  = () => {
    if (!newKey.trim()) return;
    setLevelData((p) => ({ ...p, [newKey.trim()]: { ...newEntry } }));
    setAddingNew(false);
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Exam type selector — stacks on mobile */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        {EXAM_TYPES.map((t) => (
          <button
            key={t.value}
            onClick={() => { setType(t.value); setLevelData(null); setError(''); setSaveMsg(''); setEditingKey(null); setAddingNew(false); }}
            className={`rounded-2xl p-4 text-left border-2 transition-all bg-white shadow-sm ${type === t.value ? `${t.border}` : 'border-transparent hover:border-gray-200'}`}
          >
            <p className={`text-xs font-semibold px-2 py-0.5 rounded-full inline-block mb-1 ${t.color}`}>{t.label}</p>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 sm:p-6">
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {SUGGESTED_LEVELS.map((l) => (
            <button
              ...
              className={`px-3 py-1.5 text-xs font-medium rounded-lg border transition-all ${level === l ? 'bg-blue-600 text-white border-blue-600' : 'border-gray-200 text-gray-600 hover:border-blue-300 hover:text-blue-600'}`}
            >
              {l}
            </button>
          ))}
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <button
            onClick={() => fetchData()}
            disabled={loading}
            className="flex items-center justify-center gap-2 px-5 py-2.5 bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition text-sm font-semibold disabled:opacity-60"
          >
      ...
      {error && (
        <div className="flex items-start gap-2 bg-red-50 border border-red-200 text-red-600 rounded-2xl px-4 py-3 text-sm">
          <FiAlertTriangle className="w-4 h-4 shrink-0 mt-0.5" /> {error}
        </div>
      )}
      {saveMsg && (
      ...
      {levelData && !loading && (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          {/* Table header bar */}
          <div className="px-4 sm:px-6 py-4 border-b border-gray-100 flex flex-wrap items-center gap-3 justify-between">
            ...
            <table className="w-full text-sm min-w-[500px]">
              <thead>
                <tr className="text-left text-xs text-gray-400 uppercase tracking-wide bg-gray-50 border-b border-gray-100">
                  <th className="px-4 sm:px-5 py-3 w-14">#</th>
                  {fields.map((f) => <th key={f} className="px-3 sm:px-4 py-3">{f}</th>)}
                ...
                {numericEntries.map(([key, val]) => {
                  ...
                  return (
                    <tr key={key} className={`transition-colors ${isEditing ? 'bg-blue-50' : 'hover:bg-gray-50'}`}>
                      <td className="px-4 sm:px-5 py-3 font-bold text-blue-600">{key}</td>
                      {fields.map((f) => (
                        <td key={f} className="px-3 sm:px-4 py-3">
                          {isEditing ? (
                            <FieldCell value={editBuf[f] !== undefined ? editBuf[f] : ''} onChange={(v) => setEditBuf((b) => ({ ...b, [f]: v }))} />
                          ) : (
                      ...
                      <td className="px-3 sm:px-4 py-3">
                        <div className="flex items-center justify-end gap-1">
                          {isEditing ? (
                            <>
                              <button onClick={() => commitEdit(key)} className="p-1.5 text-green-600 hover:bg-green-50 rounded-lg transition" title="Save"><FiCheck className="w-4 h-4" /></button>
                              <button onClick={cancelEdit} className="p-1.5 text-gray-400 hover:bg-gray-100 rounded-lg transition" title="Cancel"><FiX className="w-4 h-4" /></button>
                            </>
                          ) : (
                            <>
                              <button onClick={() => startEdit(key, val)} className="p-1.5 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition" title="Edit"><FiEdit2 className="w-4 h-4" /></button>
                              <button onClick={() => deleteEntry(key)} className="p-1.5 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition" title="Delete"><FiTrash2 className="w-4 h-4" /></button>
                            </>
                          )}
                        </div>
                      </td>
                  ...
                  <tr className="bg-green-50 border-t-2 border-green-200">
                    <td className="px-4 sm:px-5 py-3">
                      <input type="text" placeholder="Key" value={newKey} onChange={(e) => setNewKey(e.target.value)}
                        className="border border-gray-200 rounded-lg px-2 py-1 text-xs w-14 focus:outline-none focus:ring-2 focus:ring-green-400" />
                    </td>
                    {fields.map((f) => (
                      <td key={f} className="px-3 sm:px-4 py-3">
                        <FieldCell value={newEntry[f] !== undefined ? newEntry[f] : ''} onChange={(v) => setNewEntry((p) => ({ ...p, [f]: v }))} />
                      </td>
                    ))}
                    <td className="px-3 sm:px-4 py-3">
                      <div className="flex items-center justify-end gap-1">
                        <button onClick={commitAdd} className="p-1.5 text-green-600 hover:bg-green-100 rounded-lg transition" title="Add"><FiCheck className="w-4 h-4" /></button>
                        <button onClick={() => { setAddingNew(false); setNewKey(''); setNewEntry(blankEntry()); }} className="p-1.5 text-gray-400 hover:bg-gray-100 rounded-lg transition" title="Cancel"><FiX className="w-4 h-4" /></button>
                      </div>
                    </td>
                  </tr>
                )}
          ...
          {/* Total editor */}
          {typeof levelData.Total === 'number' && (
            <div className="px-4 sm:px-6 py-4 border-t border-gray-100 flex items-center gap-3 bg-gray-50 flex-wrap">
              <span className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Total</span>
              <input
                type="number"
                value={levelData.Total}
                onChange={(e) => setLevelData((p) => ({ ...p, Total: Number(e.target.value) }))}
